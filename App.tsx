    ...
    models.forEach(model => {
      ...
      const fetchTextureSets = async () => {
        try {
          ...
          console.log(`[App] ðŸ“¥ Received ${files.length} files from API for model: ${modelName}`);
    ...
    try {
      ...
      setModels(prev => prev.map(model => {
        ...
        const textureMatches = filteredTextures.map((tex: any) => {
          ...
          // â”€â”€ SCORED MATERIAL MATCHING â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
          let targetMat = null;
          let bestScore = -1;
        ...
        {selectedModel && (
          <div className="animate-in fade-in slide-in-from-left-4 duration-500 pointer-events-auto flex flex-col gap-1 items-start text-left">
            ...
            <button
              onClick={() => {
                ...
                navigator.clipboard.writeText(shareUrl)
                  .then(() => {
                    setToast({
                      message: language === 'he' ? '×”×§×™×©×•×¨ ×”×•×¢×ª×§ ×œ×œ×•×—!' : 'Link copied to clipboard!',
                      type: 'success'
                    });
                  })
                  .catch((err) => {
            ...
            >
              ...
              <span>{language === 'he' ? '×©×ª×£ ×ž×•×“×œ' : 'Share Model'}</span>
            </button>
          </div>
        )}
      </div>
        ...
        <div className="relative flex items-center">
          <div className={`relative flex items-center transition-all duration-300 ${isSearchOpen ? 'w-48 sm:w-64 opacity-100 mr-2' : 'w-0 opacity-0 pointer-events-none overflow-hidden'}`}>
            <input
              ...
              placeholder={language === 'he' ? '×—×¤×© ×ž×•×¦×¨...' : 'Search product...'}
              dir={isRTL ? 'rtl' : 'ltr'}
            ...
            >
              ...
              <div className="flex flex-col gap-2">
                <span className="text-[10px] font-black uppercase tracking-wider text-zinc-400 leading-none">
                  {language === 'he' ? '×©×¤×ª ×ž×ž×©×§' : 'Interface Language'}
                </span>
        ...
        {selectedModel && (
          <div className="fixed left-0 top-0 bottom-0 z-[110] pointer-events-none w-full">
            <button
              ...
              title={language === 'he' ? '×ž×™×“×¢ ×¢×œ ×”×ž×•×¦×¨' : 'Product Info'}
            >
            ...
            {/* PRODUCT INFO PANEL */}
            <div 
              className={`absolute top-1/2 -translate-y-1/2 sm:top-[calc(50%+24px)] left-0 w-[280px] sm:w-[320px] lg:w-[380px] h-[55vh] sm:h-[80vh] backdrop-blur-3xl shadow-[25px_0_80px_rgba(0,0,0,0.15)] transition-all duration-500 transform overflow-hidden flex flex-col pointer-events-auto antialiased font-sans ${
                ...
                <div className="flex items-center justify-between mb-8">
                  ...
                  <div className="flex gap-2">
                    ...
                    <button
                      onClick={() => {
                        ...
                        if (modelName) {
                          ...
                          navigator.clipboard.writeText(shareUrl)
                            .then(() => {
                              setToast({
                                message: language === 'he' ? '×§×™×©×•×¨ ×”×©×™×ª×•×£ ×”×•×¢×ª×§ ×œ×œ×•×—!' : 'Share link copied to clipboard!',
                                type: 'success'
                              });
                            })
                            .catch((err) => {
                      ...
                      title={language === 'he' ? '×”×¢×ª×§ ×§×™×©×•×¨ ×©×™×ª×•×£' : 'Copy Share Link'}
                    >
                      ...
                      <div className="flex flex-wrap gap-2 pt-1">
                        {productDetails?.category && (
                          <div className={`px-3 py-1.5 rounded-2xl text-[10px] font-black uppercase tracking-wider flex items-center gap-1.5 border shadow-sm ${
                            ...
                            <span className="opacity-60">{language === 'he' ? '×§×˜×’×•×¨×™×”:' : 'Category:'}</span>
                        ...
                        {productDetails?.subCategory && (
                          <div className={`px-3 py-1.5 rounded-2xl text-[10px] font-black uppercase tracking-wider flex items-center gap-1.5 border shadow-sm ${
                            ...
                            <span className="opacity-60">{language === 'he' ? '×§×˜×’×•×¨×™×” ×ž×©× ×™×ª:' : 'Subcategory:'}</span>
                        ...
                        {productDetails?.price !== undefined && (
                          <div className={`px-3 py-1.5 rounded-2xl text-[10px] font-black uppercase tracking-wider flex items-center gap-1.5 border shadow-sm ${
                            ...
                            <span className="opacity-60">{t.price || (language === 'he' ? '×ž×—×™×¨:' : 'Price:')}</span>
                            <span>â‚ª{productDetails.price.toLocaleString()}</span>
                          </div>
                        )}
                      </div>
                    )}
      ...
      >
        ...
        {!isIPad && (
          <button
            ...
            title={language === 'he' ? (isCatalogCollapsed ? '×¤×ª×— ×§×˜×œ×•×’' : '×¡×’×•×¨ ×§×˜×œ×•×’') : (isCatalogCollapsed ? 'Open Catalog' : 'Close Catalog')}
          >
            ...
            <div className="p-6 border-b border-zinc-800 flex items-center justify-between shrink-0">
              <div className="flex flex-col gap-1">
                <span className="text-[10px] font-black uppercase tracking-[0.2em] text-yellow-500">
                  {language === 'he' ? '×§×•××•×¨×“×™× ×˜×•×ª UV ×ž×‘×•×“×“×•×ª' : 'Isolated UV Coordinates Map'}
                </span>
            ...
            <div className="p-6 border-t border-zinc-800 bg-stone-900/60 flex flex-col gap-4 shrink-0">
              <div className="flex flex-col gap-1">
                <span className="text-[9px] font-bold uppercase tracking-widest text-zinc-500">
                  {language === 'he' ? '×–×™×”×•×™ UDIM ×•×”×ž×œ×¦×ª ×—×•×ž×¨×™×' : 'UDIM Tile & Material Recommendation'}
                </span>
                <p className="text-xs text-zinc-400 leading-relaxed font-normal">
                  {language === 'he' 
                    ? '×ž×¤×ª ×”-UV ×”×–×• × ×•×¦×¨×” ×¢×‘×•×¨ ×—×œ×§ ×ª×œ×ª-×”×ž×™×ž×“ ×”×¡×¤×¦×™×¤×™ ×”×–×” ×œ×œ× ×—×¤×™×¤×”. ×× ×• ×ž×ž×œ×™×¦×™× ×¢×œ ×˜×§×¡×˜×•×¨×ª PBR ×ž×¨×•×‘×¢×ª ×ž×•×ª××ž×ª ××™×©×™×ª ×‘×¨×–×•×œ×•×¦×™×™×ª 2048x2048 ×œ×§×‘×œ×ª ×—×“×•×ª ×ž×™×¨×‘×™×ª.' 
...
export default App;  const orbitDirectionRef = useRef(orbitDirection);
  const targetViewRef = useRef(targetView);
  const activePartMeshRef = useRef(activePartMesh);
